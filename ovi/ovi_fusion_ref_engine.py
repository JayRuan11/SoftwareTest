...
from ovi.utils.processing_utils import clean_text, preprocess_image_tensor, snap_hw_to_multiple_of_32, scale_hw_to_area_divisible, preprocess_ref_audio
import re
...
class OviFusionEngine:
    def __init__(self, config=DEFAULT_CONFIG, device=0, target_dtype=torch.bfloat16):
        ...
        self.is_base = config.get("lora_path") is not None
        load_fusion_checkpoint(model, checkpoint_path=checkpoint_path, from_meta=meta_init, is_base=True)
    ...
    def generate(self,
        ...
        params = {
            ...
            "Audio Path": ref_audio_path if ref_audio_path else "None (T2V mode)",
        ...
        try:
            ...
            if is_a2v:
                ...
                raw_ref_tensor = preprocess_ref_audio(ref_audio_path).to(self.device, dtype=torch.float32)
                # print(f"Ref audio shape: {raw_ref_tensor.shape}")
                raw_ref_tensor = raw_ref_tensor.squeeze(1)
                # print(f"Ref audio shape: {raw_ref_tensor.shape}")
                # 1 54242
                ref_audio_latents = self.vae_model_audio.wrapped_encode(raw_ref_tensor).to(self.target_dtype)
                # print(f"Ref audio latents shape: {ref_audio_latents.shape}")
                # 1 20 105
                ref_audio_latents = ref_audio_latents.transpose(1, 2)
                
                ref_audio_latents = ref_audio_latents.squeeze(0)
                # print(f"Ref audio latents shape: {ref_audio_latents.shape}")
                if self.cpu_offload:
                    self.offload_to_cpu(self.vae_model_audio)
                ref_input = [ref_audio_latents]


                
                self.audio_latent_length = ref_audio_latents.shape[0] if is_a2v else None
                raw_v_len = self.audio_latent_length / 157 * 31
                k = round((raw_v_len - 1) / 4)
                self.video_latent_length = int(k * 4 + 1)
            else:
                ref_input = None

            print()
            print(f"Video latent length: {self.video_latent_length}, audio latent length: {self.audio_latent_length}")
