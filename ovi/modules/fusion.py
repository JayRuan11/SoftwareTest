...
from ovi.modules.model import WanLayerNorm, WanModel, WanModel_Ref, WanRMSNorm, gradient_checkpointing, rope_apply
...
class FusionModel(nn.Module):
    def __init__(self, video_config=None, audio_config=None, gradient_checkpoint=False):
        super().__init__()
        has_video = True
        has_audio = True
        ...
        if audio_config is not None:
            self.audio_model = WanModel_Ref(**audio_config)
        else:
            has_audio = False
    ...
    def forward(
        ...
        if vid is None or all([x is None for x in vid]):
            ...
            return None, self.audio_model(x=audio, t=t, context=audio_context, seq_len=audio_seq_len, ref=ref, clip_fea=clip_fea_audio, y=None)
        ...
        audio, audio_e, audio_kwargs = self.audio_model.prepare_transformer_block_kwargs(
            x=audio, t=t, context=audio_context, seq_len=audio_seq_len, ref=ref, clip_fea=clip_fea_audio, y=None, first_frame_is_clean=False
        )
