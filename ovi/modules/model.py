...
class WanModel(ModelMixin, ConfigMixin):
    ...
    def prepare_transformer_block_kwargs(
        ...
        # ref = [self.ref_embedding(c.unsqueeze(0)) for c in ref]
        # x = [x_ + r_ for x_, r_ in zip(x, ref)]
...
class WanModel_Ref(WanModel):
    
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        # 定义 ref_embedding 层
        self.ref_embedding = nn.Sequential(
            ChannelLastConv1d(self.in_dim, self.dim, kernel_size=7, padding=3),
            nn.SiLU(),
            ConvMLP(self.dim, self.dim * 4, kernel_size=7, padding=3),
        )
        
        if next(self.parameters()).is_meta:
            self.ref_embedding.to_empty(device="cpu")
        self.init_ref_weights()

    
    def init_ref_weights(self):
        """
        分两步初始化：
        1. Copy: 将 patch_embedding 的权重复制给 ref_embedding
        2. Zero: 将 ref_embedding 最后一层的输出权重置为 0
        """
        try:
            self.ref_embedding.load_state_dict(self.patch_embedding.state_dict(), assign=True)
        except Exception as e:
            print(f"[WanModelWithRef] Warning: Could not copy weights perfectly: {e}")
        # self.zero_init_ref()
    ...
    def init_weights(self):
        ...
        # init output layer
        nn.init.zeros_(self.head.head.weight)
        if hasattr(self, 'ref_embedding'):
            self.init_ref_weights()
    

    def zero_init_ref(self):
        """
        将 Ref Embedding 模块的最后一层输出权重初始化为 0。
        这使得在训练刚开始时，x + ref_emb(ref) == x，相当于原模型。
        """
        last_module = self.ref_embedding[-1]
        output_layer = last_module.w2
        # 执行置 0
    ...
    def prepare_transformer_block_kwargs(
        ...
        ref_emb = []
        # 只有当 ref 不为 None 时才进行处理
        if ref is not None:
            for r in ref:
                # 2. 只有当列表中的元素 r 不为 None 时才处理 (防止传入 [None])
                if r is not None:
                    r = r.to(device)
                    ref_emb.append(self.ref_embedding(r.unsqueeze(0).clone()))
        ...
        if len(ref_emb) > 0:
            x = [x_ + r_ for x_, r_ in zip(x, ref_emb)]
    ...
    def forward(
        ...
        context,
        seq_len,
        ref=None, # 显式增加 ref 参数
        clip_fea=None,
        y=None,
        first_frame_is_clean=False
    ):
        r"""
        WanModel_Ref 的前向传播，支持参考音频注入。

        Args:
            x (List[Tensor]): 输入的视频或音频噪声 Latent。
            t (Tensor): 时间步 [B]。
            context (List[Tensor]): 文本 Embedding。
            seq_len (int): 最大序列长度。
            ref (List[Tensor], optional): 参考音频 Latent，用于音色重构。
            clip_fea (Tensor, *optional*): 视觉特征。
            y (List[Tensor], *optional*): 条件视频输入。
        """
        
        # 1. 准备参数（这里会调用 WanModel_Ref 内部重写的 prepare_transformer_block_kwargs）
        # 从而将 ref 注入到 x 中
        x, e, kwargs = self.prepare_transformer_block_kwargs(
            x=x,
            t=t,
