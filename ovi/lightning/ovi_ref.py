...
class OviFusionTrainModule(pl.LightningModule):
    ...
    def __init__(self, cfg: Dict[str, Any]):
        ...
        model.audio_model.init_ref_weights()
        ...
        # print(f"text_encoder_cpu_offload:{self.text_encoder_cpu_offload}")
        ...
        self.train_ref_prob = float(cfg.get("ref_dropout_prob", 0.1))
        ...
        self.train_ref_embedding = bool(cfg.get("train_ref_embedding", True))
    ...
    def _setup_trainable_parameters(self):
        ...
        for name, param in self.model.named_parameters():
            ...
            if "audio_model.ref_embedding" in name:
                if self.train_ref_embedding:
                    should_train = True
                    ref_emb_params += param.numel()
            elif "fusion" in name:
        ...
        print(f"  - Ref Embedding:    {ref_emb_params:>12,} params ({ref_emb_params/1e6:.2f}M) (train_ref_embedding={self.train_ref_embedding})")
    ...
    def training_step(self, batch, batch_idx):
        ...
        required_raw = ["audio", "text", "mismatched_audio", "video"]
        ...
        use_ref_audio = True
        if torch.rand(1).item() < self.train_ref_prob:
            ref_for_model = None
            use_ref_audio = False
        # else:
        #     pattern = r"<S>[\s\S]*?<E>"
        #     replacement = "<ReferenceAudio>"
        #     if isinstance(batch["text"], list) or isinstance(batch["text"], tuple):
        #         batch["text"] = [re.sub(pattern, replacement, t) for t in batch["text"]]
        #     else:
        #         batch["text"] = re.sub(pattern, replacement, batch["text"])
            # print(batch["text"])
        ...
        with torch.no_grad():
            ...
            if use_ref_audio:
                # Audio must be float32 for STFT operation (cuFFT doesn't support BFloat16)
                mismatched_audio_data = batch["mismatched_audio"].to(current_device, dtype=torch.float32)  # B C L
                # Batch encode all audio at once
                mismatched_audio_latents = self.audio_vae_model.wrapped_encode(mismatched_audio_data)  # B C L
                # Transpose to L C format expected by model: B C L -> B L C
                mismatched_audio_latents = mismatched_audio_latents.transpose(1, 2)  # B L C
                # Convert to model dtype
                mismatched_audio_latents = mismatched_audio_latents.to(dtype=model_dtype)
    
        timestep_indices = torch.randint(0, self.num_train_timesteps, (batch_size,), device=current_device)
        audio_timesteps = self.audio_scheduler.timesteps.to(current_device)[timestep_indices].to(dtype=model_dtype)
        video_timesteps = self.video_scheduler.timesteps.to(current_device)[timestep_indices].to(dtype=model_dtype)

        audio_noise = torch.randn_like(audio_latents)
        audio_noise_latents, audio_target_flow = self.get_noisy_latents_and_target(
            self.audio_scheduler, audio_latents, audio_noise, audio_timesteps
        )
        max_seq_len_audio = audio_noise.shape[1]

        video_noise = torch.randn_like(video_latents)
        video_noise_latents, video_target_flow = self.get_noisy_latents_and_target(
            self.video_scheduler, video_latents, video_noise, video_timesteps
        )
        ...
        if use_ref_audio:
            ref_for_model = [ref for ref in mismatched_audio_latents]
        else:
            ref_for_model = None        
        ...
        pred_vid, pred_audio = self.model(
            ...
            t=video_timesteps if video_for_model is not None else audio_timesteps,
            vid_context=text_emb_video_for_model,
            audio_context=text_emb_audio_for_model,
            vid_seq_len=max_seq_len_video,
            audio_seq_len=max_seq_len_audio,
            ref=ref_for_model,
            first_frame_is_clean=self.train_i2v if video_for_model is not None else False,
        )
        ...
        # self.log("train/video_loss", video_loss, on_step=True, on_epoch=True, prog_bar=False, batch_size=batch_size, sync_dist=True)
        # self.log("train/audio_loss", audio_loss, on_step=True, on_epoch=True, prog_bar=False, batch_size=batch_size, sync_dist=True)
        # self.log("train/loss", loss, on_step=True, on_epoch=True, prog_bar=True, batch_size=batch_size, sync_dist=True)

        loss = torch.tensor(0.0, device=current_device, dtype=torch.float32)

        
        # 视频分支 Loss
        if pred_vid is not None:
            pred_vid_tensor = torch.stack([p for p in pred_vid], dim=0)
            target_v = video_target_flow
            if self.train_i2v:
                target_v = target_v.clone()
                target_v[:, :, 0] = 0.0
                pred_vid_tensor[:, :, 0] = 0.0
            v_loss = self.criterion(pred_vid_tensor.float(), target_v.float())
            v_weight = self.video_loss_weight if audio_for_model is not None else 1.0
            loss = loss + v_loss * v_weight
            self.log("train/video_loss", v_loss, on_step=True, on_epoch=True, prog_bar=True, batch_size=batch_size, sync_dist=True)

        # 音频分支 Loss
        if pred_audio is not None:
            pred_audio_tensor = torch.stack([p for p in pred_audio], dim=0)
            a_loss = self.criterion(pred_audio_tensor.float(), audio_target_flow.float())
            a_weight = (1 - self.video_loss_weight) if video_for_model is not None else 1.0
            loss += a_loss * a_weight
            self.log("train/audio_loss", a_loss, on_step=True, on_epoch=True, prog_bar=True, batch_size=batch_size, sync_dist=True)
        ...
        if loss.requires_grad:
            ...
            print(f"Warning: No gradient in this step (Modality dropped). Skipping.")
            return None

    def on_before_optimizer_step(self, optimizer):
        norms = [p.grad.detach().data.norm(2) for p in self.model.parameters() if p.grad is not None]
        if norms:
            total_grad_norm = torch.max(torch.stack(norms))
            self.log("debug/total_grad_norm", total_grad_norm, on_step=True, on_epoch=False)
    ...
    def add_lora_to_model(
            ...
            # 1. 注入 LoRA (此时 PEFT 默认冻结所有 Base 参数，解冻所有 LoRA 参数)
            model = inject_adapter_in_model(lora_config, model)

            # 初始化统计变量
            lora_audio_params = 0
            lora_video_params = 0
            fusion_full_params = 0
            ref_emb_params = 0
            frozen_params = 0
            ...
            for name, param in model.named_parameters():
                ...
                is_ref_embedding = "ref_embedding" in name

                # =======================================================
                # 1. 处理本体参数 (Base Weights)
                # =======================================================
                if not is_lora_param:
                    # ref_embedding 参数
                    if is_ref_embedding and self.train_ref_embedding:
                        param.requires_grad = True
                        param.data = param.data.to(target_dtype)
                        ref_emb_params += param.numel()
                    # Fusion 层的本体参数，且开关开启 -> 全参数训练
                    elif is_fusion_layer and train_fusion:
                        param.requires_grad = True
                        param.data = param.data.to(target_dtype)
                        fusion_full_params += param.numel()
                    else:
                        # 其他本体参数 -> 冻结
            ...
            total_trainable = fusion_full_params + ref_emb_params + lora_audio_params + lora_video_params
            total_params = total_trainable + frozen_params
            print(f"[LoRA Setup] Trainable parameters breakdown:")
            print(f"  - Fusion (FULL Params):  {fusion_full_params:>12,} params ({fusion_full_params/1e6:.2f}M) (train_fusion={train_fusion})")
            print(f"  - Ref Embedding (FULL):  {ref_emb_params:>12,} params ({ref_emb_params/1e6:.2f}M) (train_ref_embedding={self.train_ref_embedding})")
            ...
            if pretrained_lora_path is not None:
                try:
                    ...
                    print(f"Loaded LoRA from {pretrained_lora_path}")
                except Exception as e:
                    print(f"Warning: failed to load pretrain LoRA checkpoint: {e}")
    ...
    def on_save_checkpoint(self, checkpoint):
        ...
        # 直接遍历模型参数，只提取需要训练的部分，避免调用 state_dict()
        lora_save_dict = {}
        for name, param in self.model.named_parameters():
            if param.requires_grad:
                # 存入 CPU 内存，并确保是深拷贝但只针对可训练参数
                lora_save_dict[name] = param.data.detach().cpu()
                
        checkpoint.update(lora_save_dict)

        # 保存 EMA 状态
        if self.use_ema and self.ema_state is not None:
            # ema_state 已经是只包含可训练参数的字典了
            checkpoint["ema_state_dict"] = {k: v.cpu() for k, v in self.ema_state.items()}
