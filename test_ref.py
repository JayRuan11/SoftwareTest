...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to reference audio file (overrides config, applied to all prompts if not using CSV)"
    )
...
def load_prompts_from_csv(csv_path: str, mode: str) -> List[Tuple[str, Optional[str], Optional[str]]]:
    """
    Load prompts from CSV file.

    Expected CSV format:
    - required: text_prompt
    - optional: image_path (for i2v)
    - optional: audio_path (for audio-driven)

    Returns:
        List of (text_prompt, image_path, audio_path) tuples
    """
    ...
    for _, row in df.iterrows():
        ...
        # 简单的文件存在性检查
        if image_path and not os.path.exists(image_path) and mode in ["i2v", "t2i2v"]:
             logging.warning(f"Image path not found: {image_path}")
        if audio_path and not os.path.exists(audio_path):
             logging.warning(f"Audio path not found: {audio_path}")

        prompts.append((text_prompt, image_path, audio_path))
...
def main():
    ...
    if prompts_file:
        ...
        if audio_path and not os.path.exists(audio_path):
             logging.error(f"Configured audio_path does not exist: {audio_path}")
        all_prompts = [(text_prompt, image_path, audio_path)]
    ...
    # Process each prompt
    for prompt_idx, (text_prompt, image_path, ref_audio_path) in enumerate(tqdm(
        this_rank_prompts,
