...
from ovi.ovi_fusion_ref_engine import OviFusionEngine
...
def load_model_with_lora(config, device, target_dtype):
    """
    加载 OVI Fusion Engine 并应用 LoRA 权重。
    """
    logging.info("Loading OVI Fusion Engine (Base Model)...")
    ...
    if lora_path and os.path.exists(lora_path):
        ...
        try:
            ...
            # 选择权重字典 (EMA 或 Regular)
            if use_ema and "ema_state_dict" in checkpoint:
                logging.info("[EMA] Using EMA weights for inference")
            ...
            if lora_state:
                ...
                logging.info(f"LoRA injected: {num_lora} modules updated.")

            ovi_engine.model.to(dtype=target_dtype) 

        except Exception as e:
            logging.error(f"Failed to load LoRA/Fusion weights: {e}")
            raise
    else:
        logging.warning("No lora_path provided in config, using BASE model only!")
...
def main(config, args): 
    ...
    text_prompts, image_paths, audio_paths_from_file = validate_and_process_user_prompt(text_prompt, image_path, audio_path=config.get("audio_path"), mode=config.get("mode"))
    ...
    if any(p is not None for p in audio_paths_from_file):
        audio_paths = audio_paths_from_file
    else:
        if audio_path is None:
            # 如果配置没写 audio_path，全为 None
            audio_paths = [None] * len(text_prompts)
        elif isinstance(audio_path, str):
            # 如果是单个字符串路径，广播给所有 prompt
            if not os.path.exists(audio_path):
                raise FileNotFoundError(f"Provided audio path does not exist: {audio_path}")
            audio_paths = [audio_path] * len(text_prompts)
        elif isinstance(audio_path, list):
            # 如果是列表，长度必须匹配
            assert len(audio_path) == len(text_prompts), f"Number of audio paths ({len(audio_path)}) must match number of text prompts ({len(text_prompts)})"
            audio_paths = audio_path
        else:
            raise ValueError("audio_path in config must be None, a string, or a list of strings")

    logging.info("Loading OVI Fusion Engine...")
    # ovi_engine = OviFusionEngine(config=config, device=device, target_dtype=target_dtype)
    
    ovi_engine = load_model_with_lora(config, device, target_dtype)
    ...
    # Load CSV data
    # all_eval_data = list(zip(text_prompts, image_paths, audio_paths))
    all_eval_data = list(zip(range(len(text_prompts)), text_prompts, image_paths, audio_paths))
    ...
    # for _, (text_prompt, image_path, ref_audio_path) in tqdm(enumerate(this_rank_eval_data)):
    for _, (global_idx, text_prompt, image_path, ref_audio_path) in tqdm(enumerate(this_rank_eval_data)):    
        ...
        print(f"text_prompt: {text_prompt}, image_path: {image_path}, ref_audio_path: {ref_audio_path}")
        for idx in range(config.get("each_example_n_times", 1)):
            ...
            if sp_rank == 0:
                video_name = f"{global_idx + 1}"
                # 如果配置中 each_example_n_times > 1，建议加上 idx 防止覆盖
                if config.get("each_example_n_times", 1) > 1:
                    video_name += f"_{idx}"
